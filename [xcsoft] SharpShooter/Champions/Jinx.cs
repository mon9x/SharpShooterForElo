...
namespace Sharpshooter.Champions
{
    public static class Jinx
    {
        ...
        static void QSwitchForUnit(AttackableUnit Unit)
        {
            ...
            if (LeagueSharp.Common.Utility.CountEnemiesInRange(Unit.Position, 200) >= SharpShooter.Menu.Item("FISHBONES", true).GetValue<Slider>().Value)
            {
                QSwitch(true);
                return;
            }
        ...
        static void AutoE()
        {
            ...
            foreach (AIHeroClient target in HeroManager.Enemies.Where(x => x.IsValidTarget(E.Range)))
            {
        ...
        static void Combo()
        {
            ...
            if (SharpShooter.Menu.Item("comboUseR", true).GetValue<Boolean>() && R.IsReady() && WLastCastedTime + 1.0 < Game.Time && !Player.Spellbook.IsAutoAttacking)
            {
                foreach (AIHeroClient Rtarget in HeroManager.Enemies.Where(x => CollisionCheck(Player, x, R.Width) && x.IsValidTarget(R.Range) && !x.IsValidTarget(Orbwalking.GetRealAutoAttackRange(Player)) && !Player.HasBuffOfType(BuffType.SpellShield) && !Player.HasBuffOfType(BuffType.Invulnerability) && R.GetPrediction(x).Hitchance >= HitChance.High && LeagueSharp.Common.Utility.GetAlliesInRange(x, 800).Where(ally => !ally.IsMe).Count() <= 1))
                {
