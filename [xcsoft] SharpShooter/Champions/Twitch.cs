...
namespace Sharpshooter.Champions
{
    public static class Twitch
    {
        ...
        static void Killsteal()
        {
            ...
            foreach (AIHeroClient target in HeroManager.Enemies.Where(x => x.IsValidTarget(E.Range) && !x.HasBuffOfType(BuffType.Invulnerability) && !x.HasBuffOfType(BuffType.SpellShield)))
            {
                if (target != null)
                {
