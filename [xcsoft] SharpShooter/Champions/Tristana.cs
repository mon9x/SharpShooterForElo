...
namespace Sharpshooter.Champions
{
    public static class Tristana
    {
        ...
        static void Killsteal()
        {
            ...
            foreach (AIHeroClient target in HeroManager.Enemies.Where(x => x.IsValidTarget(Orbwalking.GetRealAutoAttackRange(Player)) && !x.HasBuffOfType(BuffType.Invulnerability) && !x.HasBuffOfType(BuffType.SpellShield)))
            {
                if (target != null)
                {
        ...
        static void Combo()
        {
            ...
            {
                foreach (AIHeroClient Wtarget in HeroManager.Enemies.Where(x => x.IsValidTarget(W.Range) && !Player.HasBuffOfType(BuffType.SpellShield) && !Player.HasBuffOfType(BuffType.Invulnerability)))
                {
                    var Wpred = W.GetPrediction(Wtarget);

                    if(!Wpred.CastPosition.UnderTurret(true) && LeagueSharp.Common.Utility.CountEnemiesInRange(Wpred.CastPosition, 1000) <= 2)
                    {
            ...
            if (SharpShooter.Menu.Item("comboUseE", true).GetValue<Boolean>())
            {
                ...
                if (E.IsReady() && Etarget != null && Etarget.IsEnemy && !Player.Spellbook.IsAutoAttacking)
                    E.Cast((AIHeroClient)Etarget);
            }

            if (SharpShooter.Menu.Item("comboUseR", true).GetValue<Boolean>() && R.IsReady())
            {
                foreach (AIHeroClient Rtarget in HeroManager.Enemies.Where(x => x.IsValidTarget(Orbwalking.GetRealAutoAttackRange(Player)) && !Player.HasBuffOfType(BuffType.SpellShield) && !Player.HasBuffOfType(BuffType.Invulnerability)))
                {
        ...
        static void Jungleclear()
        {
            ...
            if (E.IsReady() && SharpShooter.Menu.Item("jungleclearUseE", true).GetValue<Boolean>() && !Player.Spellbook.IsAutoAttacking)
                E.Cast(Mobs[0]);
        }
    }
}
